...
// Hostyo customization: the stock modal's reference_doc row only ever
// showed Lead/Deal (via the backend's _lead/_deal), because that's all the
// upstream Twilio/Exotel flows ever linked. Our Telnyx inbound caller-ID
// lookup (_find_caller_reference in pbx_integration/telnyx.py) can also
// resolve a bare Contact when a Lead/Deal doesn't match, so this needs a
// third case - driven directly off reference_doctype/reference_docname,
// which the backend already returns on every call log regardless of type.
function referenceDocInfo(data) {
  if (data._lead) {
    return { label: 'Lead', icon: LeadsIcon, name: 'Lead', param: 'leadId', id: data._lead }
  }
  if (data._deal) {
    return { label: 'Deal', icon: Dealsicon, name: 'Deal', param: 'dealId', id: data._deal }
  }
  if (data.reference_doctype === 'Contact' && data.reference_docname) {
    return {
      label: 'Contact',
      icon: ContactsIcon,
      name: 'Contact',
      param: 'contactId',
      id: data.reference_docname,
    }
  }
  return null
}
...
const detailFields = computed(() => {
  ...
  const reference = referenceDocInfo(callLog.value.data)
  ...
  let details = [
    ...
    },
    {
      ...
      link: () => {
        if (!reference) return
        router.push({ name: reference.name, params: { [reference.param]: reference.id } })
      },
      condition: () => !!reference,
    },
    {
