          ...
          <nav class="flex flex-col">
            <template v-for="link in view.views" :key="link.label">
              <a
                v-if="link.url"
                :href="link.url"
                class="flex items-center gap-2 mx-2 my-[1.5px] px-2 py-1.5 rounded text-ink-gray-8 hover:bg-surface-gray-2 cursor-pointer text-base"
              >
                <component :is="link.icon" class="h-4 w-4 text-ink-gray-7" />
                <span v-if="!isSidebarCollapsed">{{ __(link.label) }}</span>
              </a>
              <SidebarLink
                v-else
                :icon="link.icon"
