      ...
      <template v-else>
        ...
        >
          ...
          <div
            ...
            class="pb-5 mt-px"
          >
            <div class="flex items-center gap-1.5 text-base text-ink-gray-8 mb-1">
              <span class="font-medium">{{ activity.owner_name }}</span>
              <span class="text-ink-gray-5">
                {{ activity.data.type == 'Outgoing' ? __('sent a WhatsApp message to') : __('received a WhatsApp message from') }}
              </span>
              <span class="font-medium">{{ activity.data.type == 'Outgoing' ? activity.data.to : activity.data.from_number }}</span>
              <Badge
                v-if="activity.data.status == 'failed'"
                theme="red"
                :label="activity.data.status"
              />
            </div>
            <div class="rounded-md bg-surface-gray-1 p-2.5 text-base text-ink-gray-8 whitespace-pre-wrap">
          ...
          >
            <div class="flex items-center gap-1.5 text-base text-ink-gray-8 mb-1">
              <span class="font-medium">{{ activity.owner_name }}</span>
              <span class="text-ink-gray-5">
                {{ activity.data.type == 'Outgoing' ? __('sent an SMS to') : __('received an SMS from') }}
              </span>
              <span class="font-medium">{{ activity.data.type == 'Outgoing' ? activity.data.to : activity.data.from_number }}</span>
              <Badge
                v-if="activity.data.status == 'failed'"
...
const smsMessages = createResource({
  url: 'whatsapp_integration.api.get_sms_messages',
