<template>
          ...
          <div class="flex items-center justify-start gap-5 border-b p-5">
            ...
            <div class="flex flex-col gap-2.5 truncate">
              ...
              <div class="flex gap-1.5">
                ...
                <!-- ✅ WHATSAPP BUTTON -->
                <Button
                  :tooltip="__('Send WhatsApp Message')"
                  :icon="WhatsAppIcon"
                  @click="
                    (doc.phone || doc.mobile_no)
                      ? openWhatsAppBox()
                      : toast.error(
                          __('Please set a mobile number to send WhatsApp message'),
                        )
                  "
                />
  ...
  <!-- ✅ WHATSAPP MODAL (same pattern as Email composer, built with frappe-ui Dialog) -->
  <Dialog v-model="showWhatsAppBox" :options="{ title: __('Send WhatsApp Message'), size: 'lg' }">
...
// ✅ WhatsApp: open the modal
function openWhatsAppBox() {
  whatsappMessage.value = ''
  showWhatsAppBox.value = true
}

// ✅ WhatsApp: send message via backend API (falls back to wa.me if API is unavailable)
async function sendWhatsApp() {
  ...
  const number = doc.value.phone
  if (!number) {
    toast.error(__('No mobile number found'))
    return
  }

  sendingWhatsApp.value = true
  try {
    // Preferred: send through your backend WhatsApp integration
    // (e.g. whatsapp_integration.api.send_whatsapp) so the message is logged
    // against the Lead and actually delivered via WhatsApp Cloud API/Twilio.
    await call('whatsapp_integration.api.send_whatsapp', {
    ...
    // Fallback: open wa.me chat with the message pre-filled if no backend API exists
    console.error(err)
