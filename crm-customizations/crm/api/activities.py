...
def get_deal_activities(name: str):
	...
	activities = activities + get_linked_whatsapp_messages(name) + get_linked_sms_messages(name)
	if lead:
		activities = activities + get_linked_whatsapp_messages(lead) + get_linked_sms_messages(lead)
...
def get_linked_whatsapp_messages(name: str):
	messages = frappe.db.get_all(
		"WhatsApp Message",
		filters={"reference_doctype": ["in", ["CRM Lead", "CRM Deal"]], "reference_name": name},
		fields=["name", "type", "status", "message", "to", "from_number", "content_type", "attach", "creation", "owner"],
	)
	activities = []
	for m in messages:
		activities.append({
...
def get_linked_sms_messages(name: str):
	messages = frappe.db.get_all(
		"SMS Message",
		filters={"reference_doctype": ["in", ["CRM Lead", "CRM Deal"]], "reference_name": name},
		fields=["name", "type", "status", "message", "to", "from_number", "creation", "owner"],
	)
	activities = []
	for m in messages:
		activities.append({
